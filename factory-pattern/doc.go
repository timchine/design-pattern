...
=============工厂模式模式===================
	...
	2. simpleFactory.go 文件里是网上多数说这种是工厂模式，但是其实并不是，对比下面我讲的你就会感觉的到。
		...
		=>> 抽象工厂模式
		抽象工厂是基于上面的代码改造的。
		ingredient.go 文件定义了一个借口。PizzaIngredientFactory 如定义提供一个接口，用于创建相关或依赖对象的家族
		而这个家族用来提供面团和酱汁。我美国的分店不可能要到中国来拿原料，所以我们定义了两个结构体一个纽约的原料工厂和中国的原料工厂。两个工厂分别实现
		为制作pizza提供不同的原料。
在这里说明一下这两个模式粗糙的实现了一下。只是让大家理解两种设计模式，不要纠结代码当中的一些细节。实际业务中不可能定义这么多结构体。
又是这个披萨，又是那个披萨。但是基本的用法告诉大家就可以了
*/